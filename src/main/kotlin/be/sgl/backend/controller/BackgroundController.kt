...
class BackgroundController(
    ...
    @Operation(
        ...
        responses = [
            ApiResponse(responseCode = "200", description = "Ok", content = [Content(array = ArraySchema(schema = Schema(implementation = RemoteFile::class)))])
        ]
    )
    fun getBackgrounds(): List<RemoteFile> {
    ...
    @Operation(
        ...
        responses = [
            ApiResponse(responseCode = "200", description = "Background uploaded", content = [Content(schema = Schema(implementation = RemoteFile::class))]),
            ApiResponse(responseCode = "500", description = "Image error", content = [Content(schema = Schema(implementation = ApiErrorResponse::class))])
        ]
    )
    fun addBackground(@RequestParam("file") file: MultipartFile): RemoteFile {
        return RemoteFile(imageService.upload(BACKGROUND, file))
    }

    @DeleteMapping("/{filename}", produces = [APPLICATION_JSON_VALUE])
    @OnlyAdmin
    @Operation(
        ...
        responses = [
            ...
            ApiResponse(responseCode = "400", description = "Background doesn't exist", content = [Content(schema = Schema(implementation = ApiErrorResponse::class))]),
            ApiResponse(responseCode = "500", description = "Image error", content = [Content(schema = Schema(implementation = ApiErrorResponse::class))])
        ]
    )
