package be.sgl.backend.controller.organization
...
class OrganizationController(
    ...
    @GetMapping("/owner", produces = [APPLICATION_JSON_VALUE])
    @Public
    @Operation(
        summary = "Get the owning organization",
        description = "Returns the single organization behind this website.",
        responses = [
            ApiResponse(responseCode = "200", description = "Ok", content = [Content(schema = Schema(implementation = OrganizationDTO::class))]),
            ApiResponse(responseCode = "409", description = "Not configured", content = [Content(schema = Schema(implementation = ApiErrorResponse::class))])
        ]
    )
    fun getOwner(): OrganizationDTO {
        return organizationService.getOwner()
    }

    @GetMapping("/certifier", produces = [APPLICATION_JSON_VALUE])
    @Public
    @Operation(
    ...
    fun getCertifier(): OrganizationDTO {
        return organizationService.getCertifier()
    }
    ...
    @Operation(
        ...
        responses = [
            ApiResponse(responseCode = "201", description = "Organization created", content = [Content(schema = Schema(implementation = OrganizationDTO::class))]),
            ApiResponse(responseCode = "400", description = "Bad organization format", content = [Content(schema = Schema(implementation = ApiErrorResponse::class))]),
    ...
    @Operation(
        ...
        responses = [
            ApiResponse(responseCode = "200", description = "Organization updated", content = [Content(schema = Schema(implementation = OrganizationDTO::class))]),
            ApiResponse(responseCode = "400", description = "Bad organization format", content = [Content(schema = Schema(implementation = ApiErrorResponse::class))]),
